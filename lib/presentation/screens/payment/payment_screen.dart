...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  void initState() {
    ...
    if (Get.arguments == null || Get.arguments is! List || (Get.arguments as List).isEmpty) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        Get.back();
        CustomSnackBar.error(errorList: ['Invalid payment data']);
      });
      return;
    }
    ...
    final rideArg = args[0];
    
    if (rideArg is! RideModel) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        Get.back();
        CustomSnackBar.error(errorList: ['Invalid ride data']);
      });
      return;
    }
    ...
    WidgetsBinding.instance.addPostFrameCallback((time) {
      ...
      controller.updateTips("${args.length > 1 ? args[1] : ''}");
    });
  }
