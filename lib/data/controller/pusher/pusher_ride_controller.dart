...
class PusherRideController extends GetxController {
  ...
  void _handleLiveLocation(PusherResponseModel enventResponse) {
    ...
    if (rideDetailsController.ride.status == AppStatus.RIDE_ACTIVE.toString() || rideDetailsController.ride.status == AppStatus.RIDE_RUNNING.toString()) {
      ...
      rideDetailsController.mapController.updateDriverLocation(
        ...
        isRunning: rideDetailsController.ride.status == AppStatus.RIDE_RUNNING.toString(),
      );
    }
  }
  ...
  void _handleBidAccept(PusherResponseModel enventResponse) {
    ...
    if (ride != null) {
      ...
      if (ride.driver != null) {
        rideDetailsController.mapController.updateDriverInfo(
      ...
      if (lat != 0.0 && lng != 0.0) {
        ...
        if (pickupLatLng.latitude != 0 && pickupLatLng.longitude != 0) {
          rideDetailsController.mapController.calculateETAManually(
          ...
          if (eta.isNotEmpty) {
            Get.snackbar(
              '✅ Driver Assigned!',
              'Your driver is on the way\nETA: $eta${distance.isNotEmpty ? " • Distance: $distance" : ""}',
    ...
    try {
      ...
      if (currentDriver != null) {
        final lat = StringConverter.formatDouble(
          currentDriver['latitude']?.toString() ?? '0', 
          precision: 10
        );
        final lng = StringConverter.formatDouble(
          currentDriver['longitude']?.toString() ?? '0',
          precision: 10
        );
        ...
        if (lat != 0.0 && lng != 0.0) {
          ...
          rideDetailsController.mapController.showSearchingDriverMarker(
      ...
      Get.snackbar(
        ...
        'Contacting driver $queuePosition of $totalDrivers nearby...',
        snackPosition: SnackPosition.TOP,
      ...
      Get.snackbar(
        '🔍 Searching for Driver',
        'Contacting nearest available driver...',
        snackPosition: SnackPosition.TOP,
        duration: const Duration(seconds: 3),
        backgroundColor: Get.theme.colorScheme.primaryContainer,
        colorText: Get.theme.colorScheme.onPrimaryContainer,
  ...
  void _handleNewBid(PusherResponseModel enventResponse) {
    ...
    if (bid != null) {
      ...
      CustomBidDialog.newBid(
        ...
        serviceImagePath: '${rideDetailsController.serviceImagePath}/${enventResponse.data?.service?.image}',
        totalRideCompleted: enventResponse.data?.driverTotalRide ?? '0',
      );
  ...
  void _handleRideCanceled(PusherResponseModel enventResponse) {
    ...
    if (ride != null) {
      ...
      if (canceledBy == 'driver') {
        ...
        Get.snackbar(
          '🚫 Ride Canceled',
          'Driver canceled the ride: $cancelReason',
          snackPosition: SnackPosition.TOP,
